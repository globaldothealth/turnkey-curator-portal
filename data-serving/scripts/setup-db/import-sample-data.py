...
def convert_case(case):
    ...
    if travelHistory := case.get("travelHistory"):
        ...
        converted["travelHistory"]["travelHistoryEntry"] = convert_dict_to_date(date)
    events = case.get("events", {})
    if date := events.get("dateEntry"):
        converted["events"]["dateEntry"] = convert_dict_to_date(date)
    if date := events.get("dateOnset"):
        converted["events"]["dateOnset"] = convert_dict_to_date(date)
    if date := events.get("dateReported"):
        converted["events"]["dateReported"] = convert_dict_to_date(date)
    if date := events.get("dateLastModified"):
        converted["events"]["dateLastModified"] = convert_dict_to_date(date)
    if date := events.get("dateConfirmation"):
        converted["events"]["dateConfirmation"] = convert_dict_to_date(date)
    if date := events.get("dateOfFirstConsult"):
        converted["events"]["dateOfFirstConsult"] = convert_dict_to_date(date)
    if date := events.get("dateHospitalization"):
        converted["events"]["dateHospitalization"] = convert_dict_to_date(date)
    if date := events.get("dateDischargeHospital"):
        converted["events"]["dateDischargeHospital"] = convert_dict_to_date(date)
    if date := events.get("dateAdmissionICU"):
        converted["events"]["dateAdmissionICU"] = convert_dict_to_date(date)
    if date := events.get("dateDischargeICU"):
        converted["events"]["dateDischargeICU"] = convert_dict_to_date(date)
    if date := events.get("dateIsolation"):
        converted["events"]["dateIsolation"] = convert_dict_to_date(date)
    if date := events.get("dateDeath"):
        converted["events"]["dateDeath"] = convert_dict_to_date(date)
    if date := events.get("dateRecovered"):
        converted["events"]["dateRecovered"] = convert_dict_to_date(date)
...
def main():
    ...
    with open("../../samples/day0cases.json") as f:
        ...
        request_body = {"cases": converted_cases}

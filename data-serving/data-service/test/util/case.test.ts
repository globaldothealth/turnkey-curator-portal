...
describe('Case', () => {
    ...
    it('handles any undefined fields', async () => {
        ...
        expect(
            denormalizedCase['events.firstClinicalConsultation.date'],
        ).toEqual('');
        ...
        expect(
            denormalizedCase['preexistingConditions.hasPreexistingConditions'],
        ).toEqual('');
        expect(denormalizedCase['preexistingConditions.values']).toEqual('');
        expect(
            denormalizedCase['revisionMetadata.creationMetadata.curator'],
        ).toEqual('');
        expect(
            denormalizedCase['revisionMetadata.creationMetadata.date'],
        ).toEqual('');
        expect(
            denormalizedCase['revisionMetadata.creationMetadata.notes'],
        ).toEqual('');
        expect(
            denormalizedCase['revisionMetadata.editMetadata.curator'],
        ).toEqual('');
        ...
        expect(
            denormalizedCase['travelHistory.travel.dateRange.start'],
        ).toEqual('');
        expect(
            denormalizedCase['travelHistory.travel.location.country'],
        ).toEqual('');
    ...
    it('denormalizes events fields', async () => {
        ...
        expect(
            denormalizedCase['events.firstClinicalConsultation.date'],
        ).toEqual(consultEvent.dateRange.end);
        expect(denormalizedCase['events.onsetSymptoms.date']).toEqual(
            onsetEvent.dateRange.end,
        );
        expect(denormalizedCase['events.outcome.date']).toEqual(
            outcomeEvent.dateRange.end,
        );
        expect(denormalizedCase['events.outcome.value']).toEqual(
            outcomeEvent.value,
        );
    ...
    it('denormalizes revision metadata fields', async () => {
        ...
        expect(
            denormalizedCase['revisionMetadata.creationMetadata.notes'],
        ).toEqual('added more information');
        expect(
            denormalizedCase['revisionMetadata.editMetadata.curator'],
        ).toEqual('Jim');
        expect(denormalizedCase['revisionMetadata.editMetadata.date']).toEqual(
    ...
    it('denormalizes travel history fields', async () => {
        ...
        expect(
            denormalizedCase['travelHistory.travel.dateRange.start'],
        ).toEqual('2020-05-01,2020-06-01');
        expect(
            denormalizedCase['travelHistory.travel.location.country'],
        ).toEqual('Azerbaijan,Russia');
        expect(denormalizedCase['travelHistory.travel.location.name']).toEqual(
            'Baku,Sochi',
        );
        expect(denormalizedCase['travelHistory.travel.methods']).toEqual(
            'train,taxi;plane,taxi',
        );
        expect(denormalizedCase['travelHistory.travel.purpose']).toEqual(
            'business,pleasure',
        );
